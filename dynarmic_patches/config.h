...
struct UserCallbacks : public TranslateCallbacks {
    ...
    /// Query for a fast-path SVC handler. Called at JIT compile time when the
    /// JIT emits code for an SVC instruction. If this returns a value, the JIT
    /// inlines a direct call to the supplied function pointer instead of
    /// dispatching through CallSVC. Return std::nullopt to keep the standard
    /// CallSVC path (the default).
