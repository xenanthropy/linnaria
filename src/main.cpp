    ...
    try {
        ...
        */
        ...
        while (running) {
            ...
            if (main_thread.is_alive) {
                ...
                if (halt == Dynarmic::HaltReason::UserDefined1) {
                    ...
                    main_thread.cpsr = 0x00000030; // <--- ADD THIS

                }
                else if (halt == Dynarmic::HaltReason::UserDefined2) {
                    cpu.ClearHalt(Dynarmic::HaltReason::UserDefined2);
                    // Main thread yielded via nanosleep. Fall through to worker scheduling.
                }
            }

            // --- 3. Run Background Workers (interleaved with main thread yields) ---
            for (size_t i = 0; i < worker_threads.size(); i++) {
                auto& t = worker_threads[i];
                if (!t.is_alive) continue;

                for (int r = 0; r < 16; r++) cpu.Regs()[r] = t.regs[r];
                cpu.SetCpsr(t.cpsr);
                ...
                if (halt == Dynarmic::HaltReason::UserDefined2) {
                    cpu.ClearHalt(Dynarmic::HaltReason::UserDefined2);
                } else if (halt == Dynarmic::HaltReason::UserDefined1 ||
                           cpu.Regs()[15] == 0xFFFFFFFF ||
                           cpu.Regs()[15] == 0xFFFFFFFE) {
                    std::cout << "[Threading] Worker " << (i+1) << " exited." << std::endl;
                    t.is_alive = false;
                }

                for (int r = 0; r < 16; r++) t.regs[r] = cpu.Regs()[r];
                t.cpsr = cpu.Cpsr();
            }
        ...
        while (running) {
            ...
            // 1. Process OS Events (Like clicking the 'X' button to close the window)
            SDL_Event event;
            while (SDL_PollEvent(&event)) {
            ...
            // 2. Run the Main Thread (The Game Engine's Update & Draw)
            ExecuteGameFunction(cpu, memory, loader, 
                "Java_com_codeglue_terraria_OctarineBridge_nativeOnUpdate", 
                { env_ptr, 0, 1, 1 } // Note: Remember to keep using your float bitcast here if you fixed that earlier!
            );

            // 3. Time-slice the Background Workers
            for (size_t i = 0; i < worker_threads.size(); i++) {
            ...
            // 4. Push the rendered frame to the physical monitor!
            SDL_GL_SwapWindow(window);

            // 5. Cap the framerate to ~60 FPS (16.6 milliseconds per frame)
            uint32_t frame_time = SDL_GetTicks() - frame_start;
            if (frame_time < 16) {
                SDL_Delay(16 - frame_time);
            }
