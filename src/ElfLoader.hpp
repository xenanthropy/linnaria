...
public:
    ...
    std::vector<uint32_t> GetConstructors() {
        std::vector<uint32_t> out;
        if (dyn.init) out.push_back(dyn.init);
        if (dyn.init_array && dyn.init_arraysz) {
            size_t count = dyn.init_arraysz / sizeof(uint32_t);
            for (size_t i = 0; i < count; ++i) {
                uint32_t addr = mem.Read32(dyn.init_array + i * sizeof(uint32_t));
